...
export const createAndSendTransaction = async (decryptedSeedPhrase, password, amount, destAddress, our_wallet, nameId, nameValue, addrType) => {
    ...
    const txResponse = await sendToAddress(addressKeys, destAddress, changeAddress, amount, selectedInputs, nameId, nameValue, addrType)     //chai.expect(addressesOfBob[0].address.substring(0,1)).to.not.be.uppercase
